...
for x in tqdm(files, total=len(files)):
    ...
    cfts_record["resolver"] = (
        f"https://schnitzler-tagebuch.acdh.oeaw.ac.at/{record['id']}.html"
    )
    ...
    record["persons"] = [
        ...
        for x in doc.any_xpath(".//tei:person/tei:persName[1]")
    ]
    cfts_record["persons"] = record["persons"]
    record["places"] = [
        " ".join(" ".join(x.xpath(".//text()")).split())
        for x in doc.any_xpath(".//tei:place[@xml:id]/tei:placeName[1]")
    ]
    cfts_record["places"] = record["places"]
    record["works"] = [
        " ".join(" ".join(x.xpath(".//text()")).split())
        for x in doc.any_xpath(".//tei:listBibl//tei:bibl[@xml:id]/tei:title[1]")
    ]
